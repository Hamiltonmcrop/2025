...
struct Truck{
    ...
    unordered_map<uint32_t, Item> item_map;
    vector<Item> list_items;
      // Using unordered_map for fast item lookup by ID
};
...
void readInput(int argc, char* argv[]){
    ...
    if(argc != 3){
        cerr << "Usage: " << argv[0] << " <n_items> <n_trucks>" << endl;
        exit(1);
    }

    n_items = atoi(argv[1]);
    n_trucks = atoi(argv[2]);
    ...
    for(uint32_t i=1; i<=n_trucks; ++i){
        uint32_t width, height, cost;
        cin >> width >> height >> cost;
        truck[i].width = width;
        truck[i].height = height;
        truck[i].cost = cost;
        truck[i].id = i;
    ...
    // Populate item_map for faster item lookup by ID
    for(uint32_t i = 1; i <= n_items; ++i) {
        truck[item[i].truck_id].item_map[item[i].id] = item[i];
    }
}
...
void printSolution(){
    ...
    for(uint32_t i=1; i<=n_items; ++i){
        cout << "Item " << item[i].id << ": ";
        if(item[i].truck_id == 0){
            cout << "Not placed" << endl;
        } else {
            cout << "Truck " << item[i].truck_id << ", Corner: (" << item[i].corner_x << ", " << item[i].corner_y << "), Rotated: " << (item[i].rotated ? "Yes" : "No") << endl;
        }
    }
}
